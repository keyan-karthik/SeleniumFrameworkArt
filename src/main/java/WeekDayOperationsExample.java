 ...
 * java.util.HashMap; import java.util.Map; import java.util.Scanner;
 * 
 * public class WeekDayOperationsExample {
 * 
 * static int weekdayno; static dayoftheweek weekday[]=dayoftheweek.values();
 * enum dayoftheweek { MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY;
 * }
 * 
 * private static int weekdayoperation(int n) { int amount=n%7; return
 * weekday[((weekdayno-1)+amount+7)%7]; } public static void main(String[] args)
 ...
 * 
 * 
 * System.out.println("Enter the weekday in number"); weekdayno=sc.nextInt();
 * 
 * System.out.println("Corresponding weekday for this number is:"+weekday[
 * weekdayno-1]);
 * System.out.println("Enter the no to do addition or subraction operation");
 * int n=sc.nextInt(); int result=
 * 
